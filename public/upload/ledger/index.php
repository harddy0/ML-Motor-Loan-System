...
<style>
    ...
    <div id="dropZone" class="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center transition-all hover:border-slate-500 hover:bg-slate-50/50 mx-10 mb-10 shadow-sm flex-1 min-h-[430px] overflow-hidden no-scrollbar">
        ...
        <div onclick="document.getElementById('fileInput').click()" class="relative mb-6 cursor-pointer group">
            <div class="w-20 h-24 bg-slate-50 rounded-xl relative border-2 border-slate-200 overflow-hidden group-hover:border-blue-500 group-hover:bg-white transition-all duration-300">
                <div class="absolute top-0 right-0 w-6 h-6 bg-slate-200 rounded-bl-lg group-hover:bg-blue-500 transition-colors"></div>
                <div class="absolute bottom-0 left-0 right-0 h-8 bg-slate-100 flex items-center justify-center group-hover:bg-blue-500 transition-colors">
                    <span class=" font-black text-slate-400 group-hover:text-white">CSV</span>
                </div>
        ...
        <div class="mb-8 text-center shrink-0">
            <div class="inline-flex items-center bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
                ...
                <span id="displayFileName" class=" text-blue-600 ">No file selected</span>
            </div>
        </div>

        <div id="buttonContainer" class="hidden flex items-center gap-4 shrink-0">
            <button onclick="openImportModal()" class="px-4 py-1 bg-blue-600 text-white rounded-full shadow-sm hover:shadow-lg hover:brightness-110 transition-all duration-200 active:scale-95">
                Import Ledger
            </button>
        ...
        <div class="bg-[#eeeeee] w-full max-w-5xl rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            <div class="p-6 overflow-x-auto flex-1">
                <table class="w-full text-left border-collapse bg-white rounded-lg overflow-hidden shadow-sm">
                    <thead>
                        <tr class="text-blue-800 font-black">
                            <th class="px-4 py-3 text-center border-b">Loan ID</th>
                            <th class="px-4 py-3 text-center border-b">Inst No.</th>
                            <th class="px-4 py-3 text-center border-b">Scheduled Date</th>
                            <th class="px-4 py-3 text-center border-b">Principal</th>
                            <th class="px-4 py-3 text-center border-b">Interest</th>
                            <th class="px-4 py-3 text-center border-b">Total Pmt</th>
                            <th class="px-4 py-3 text-center border-b">Status</th>
                        </tr>
                    </thead>
                    <tbody id="preview-body" class="text-slate-800">
                        <?php
                        // Dummy preview data mapped to Amortization_Ledger table schema
                        $preview_data = [
                            ['loan_id' => '1045', 'inst_no' => '1', 'date' => '02/15/2026', 'principal' => '3000.00', 'interest' => '825.00', 'total' => '3825.00', 'status' => 'PENDING'],
                            ['loan_id' => '1045', 'inst_no' => '2', 'date' => '02/28/2026', 'principal' => '3000.00', 'interest' => '825.00', 'total' => '3825.00', 'status' => 'PENDING'],
                        ];
                        foreach($preview_data as $row): ?>
                        <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            <td class="px-4 py-2 border-x text-center bg-blue-50/50"><?= $row['loan_id'] ?></td>
                            <td class="px-4 py-2 border-x text-center"><?= $row['inst_no'] ?></td>
                            <td class="px-4 py-2 border-x text-center bg-blue-50/50"><?= $row['date'] ?></td>
                            <td class="px-4 py-2 border-x text-right"><?= $row['principal'] ?></td>
                            <td class="px-4 py-2 border-x text-right bg-blue-50/50"><?= $row['interest'] ?></td>
                            <td class="px-4 py-2 border-x text-right font-black italic"><?= $row['total'] ?></td>
                            <td class="px-4 py-2 border-x text-center"><span class="px-2 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-bold"><?= $row['status'] ?></span></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
    ...
    <div id="importResultsModal" class="fixed inset-0 z-[60] hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-8 relative overflow-hidden flex flex-col max-h-[90vh]">
            <div class="text-center mb-6 shrink-0">
                ...
                <h3 class="text-slate-800 font-black text-xl" id="result-title">Ledger Upload Complete</h3>
                <p class="text-slate-500 text-xs font-bold mt-1" id="result-subtitle">Successfully generated new amortization entries.</p>
            </div>
            ...
            <div class="flex justify-center shrink-0">
                <button onclick="window.location.href='../../reports/ledger/index.php'" class="px-10 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-full font-black shadow-md transition-all duration-200">
                    View Ledgers
                </button>
            </div>
        </div>
...
<script src="../../assets/js/upload_ledger.js?v=<?php echo time(); ?>"></script>
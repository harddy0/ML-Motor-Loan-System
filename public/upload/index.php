...
<style>
    ...
    <div id="dropZone" class="bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center transition-all hover:border-slate-500 hover:bg-slate-50/50 mx-10 mb-10 shadow-sm flex-1 min-h-[430px] overflow-hidden no-scrollbar">
        ...
        <div onclick="document.getElementById('fileInput').click()" class="relative mb-6 cursor-pointer group">
            ...
            <div class="absolute inset-0 bg-blue-500/5 rounded-xl scale-110 opacity-0 group-hover:opacity-100 transition-all duration-500 -z-10"></div>
        </div>

        <div class="mb-2 shrink-0">
            <label for="fileInput" class="cursor-pointer">
                <h2 class="text-center">
                    Drag & Drop file here or <span class="hover:underline">Choose File</span>
                </h2>
            </label>
        ...
        <div class="mb-8 text-center shrink-0">
            <div class="inline-flex items-center">
                ...
                <span id="displayFileName">No file selected</span>
            </div>
        </div>

        <div id="buttonContainer" class="hidden flex items-center gap-4 shrink-0">
            <button onclick="openImportModal()" class=" px-3 py-2 bg-[#ce1126] text-white rounded-full shadow-sm hover:shadow-lg hover:brightness-110 transition-all duration-200 active:scale-50">
                Import Data
            </button>
            <button onclick="window.location.reload()" class=" px-3 py-2 bg-white border border-slate-200 rounded-full hover:bg-slate-50 hover:shadow-sm transition-all duration-200 active:scale-50">
                cancel
            </button>
        </div>
    </div>
    ...
    <div id="importPreviewModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-slate-200 w-full max-w-5xl rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
                ...
                <table class="w-full text-left border-collapse bg-white rounded-lg overflow-hidden shadow-sm">
                    ...
                    <tbody id="preview-body">
                        ...
                        <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-4 py-2 border-x text-center bg-yellow-100/50 italic"><?= $row['amount'] ?></td>
            ...
            <div class="p-6 pt-0 flex justify-end gap-4">
                <button 
                    ...
                    class="px-8 py-3 bg-red-600 text-white rounded-full shadow-sm hover:shadow-lg hover:brightness-110 transition-all duration-200 ease-in-out active:scale-95 active:shadow-inner">
                    PROCEED
                </button>
                <button onclick="closeImportModal()" class="px-8 py-3 bg-white/20 border border-slate-200 rounded-full hover:bg-slate-50 hover:border-slate-300 hover:shadow-sm transition-all duration-200 active:scale-95">
                    Cancel
                </button>
            </div>
        </div>
    ...
    <div id="importResultsModal" class="fixed inset-0 z-[60] hidden items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-8 relative overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div class="text-center mb-6 shrink-0">
                ...
                <h3 id="result-title">Upload Complete</h3>
                <p class="mt-1" id="result-subtitle">Successfully processed 0 records.</p>
            </div>
            ...
            <div id="result-details-container" class="hidden flex-1 overflow-y-auto min-h-0 bg-slate-50 rounded-xl p-5 border border-slate-200 mb-6 custom-scrollbar">
                <h4 class="mb-3">Notices & Discrepancies</h4>
                <ul id="result-issues-list" class="space-y-2">
                </ul>
            </div>

            <div class="flex justify-center shrink-0">
                <button onclick="window.location.href='../reports/running_receivables/index.php'" class="px-10 py-3 bg-red-600 hover:bg-red-700 text-white rounded-full shadow-md transition-all duration-200">
                    View Reports
                </button>
            </div>
        </div>

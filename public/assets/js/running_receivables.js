...
function getReceivablesPrintHeaderInfo() {
    ...
    const asOf = `${selectedMonth} ${currentDay}, ${selectedYear}`.trim();
...
function buildReceivablesExportHeaderHtml() {
    ...
    const brandText = headerData.brandText
        ? `<span class="brand-text">${escapeReceivablesHtml(headerData.brandText)}</span>`
        : '';
...
function getReceivablesExportHeaderData() {
    ...
    const brandText = container.querySelector('[name="center"] span')?.textContent?.trim() || '';

    return {
        leftLogoSrc: resolveReceivablesAssetUrl(leftLogoRaw),
        centerLogoSrc: resolveReceivablesAssetUrl(centerLogoRaw),
        brandText
    };
}

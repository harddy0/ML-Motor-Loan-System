...
<div class="flex flex-col gap-6 h-full min-h-[600px]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white border-t-4 border-yellow-500 rounded-xl shadow-sm p-5 relative overflow-hidden group hover:shadow-md transition-all">
            ...
            <h3 class="text-slate-400 font-bold text-[13px] uppercase tracking-[0.2em] mb-1">Total Loaned</h3>
        ...
        <div class="bg-white border-t-4 border-green-500 rounded-xl shadow-sm p-5 relative overflow-hidden group hover:shadow-md transition-all">
            ...
            <h3 class="text-slate-400 font-bold text-[13px] uppercase tracking-[0.2em] mb-1">Payment</h3>
        ...
        <div class="bg-white border-t-4 border-[#e11d48] rounded-xl shadow-sm p-5 relative overflow-hidden group hover:shadow-md transition-all">
            ...
            <h3 class="text-slate-400 font-bold text-[13px] uppercase tracking-[0.2em] mb-1">Total Outstanding</h3>
        ...
        <div class="bg-white border-t-4 border-[#1d7fe1] rounded-xl shadow-sm p-5 relative overflow-hidden group hover:shadow-md transition-all">
            ...
            <h3 class="text-slate-400 font-bold text-[13px] uppercase tracking-[0.2em] mb-1">Income (This Month)</h3>
    ...
    <div class="flex-1 bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden flex flex-col">
        ...
        <div class="bg-slate-50 px-6 py-3 border-b border-slate-200 flex justify-between items-center shrink-0">
            <h2 class="text-slate-800 font-bold text-[12px] tracking-widest uppercase flex items-center gap-2">
                Active Portfolio Records
            </h2>
            <span class="text-[12px] font-black text-slate-400 uppercase">Showing <?= count($receivables) ?> Accounts</span>
        </div>
        ...
        <div class="overflow-x-auto custom-scrollbar flex-1 rounded-b-xl border border-slate-100">
            <table class="w-full text-left border-collapse whitespace-nowrap" id="receivablesTable">
                <thead>
                    <tr class="bg-slate-200 text-slate-800 sticky top-0 z-10">
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider border-r border-white/10 text-center w-24">Employee ID</th>
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider border-r border-white/10 min-w-[180px]">Borrower</th>
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider border-r border-white/10 text-center w-28">Region</th>
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider border-r border-white/10 text-center w-28">Loan Granted</th>
                        
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider text-right border-r border-white/10 border-t-4 border-yellow-500">
                            Amount Loan<br><span class="text-[12px] text-slate-800 font-normal">(Base Principal)</span>
                        </th>
                        
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider text-right border-r border-white/10 ">
                            Monthly Principal<br><span class="text-[12px] text-slate-800 font-normal">(This Period)</span>
                        </th>
                        
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider text-right border-r border-white/10 ">
                            Prior Principal<br><span class="text-[12px] text-slate-800 font-normal">(Excl. This Period)</span>
                        </th>
                        
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider text-right border-r border-white/10 border-t-4 border-green-500">
                            Payment<br><span class="text-[12px] text-slate-800 font-normal">(Total Principal)</span>
                        </th>
                        
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider text-right border-r border-white/10 border-t-4 border-[#e11d48] ">
                            Outstanding Bal<br><span class="text-[12px] text-slate-800 font-normal">(Remaining)</span>
                        </th>
                        
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider text-right border-r border-white/10 border-t-4 border-[#1d7fe1]">
                            Monthly Income<br><span class="text-[12px] text-slate-800 font-normal">(Interest)</span>
                        </th>
                        
                        <th class="px-4 py-4 text-[13px] font-black uppercase tracking-wider text-center w-24">Status</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    <?php if(empty($receivables)): ?>
                        <tr>
                            <td colspan="11" class="px-6 py-8 text-center text-slate-800 text-[13px] font-bold uppercase">
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-slate-200 transition-all duration-200 group cursor-default">
                            ...
                            <td class="px-4 py-3 text-[14px] font-medium text-slate-800 border-r border-slate-50 text-center italic">
                                <?= htmlspecialchars($row['employe_id']) ?>
                            </td>

                            <td class="px-4 py-3 text-[14px] font-black text-slate-800 border-r border-slate-100 uppercase">
                                <?= htmlspecialchars($row['name']) ?>
                            </td>

                            <td class="px-4 py-3 text-[14px] font-bold text-slate-500 border-r border-slate-100 text-center uppercase">
                                <?= htmlspecialchars($row['region'] ?? 'N/A') ?>
                            </td>

                            <td class="px-4 py-3 text-[14px] font-bold text-slate-800 border-r border-slate-100 text-center">
                                <?= ($row['loan_granted'] === 'No Date') ? 'No Date' : date('Y-m-d', strtotime($row['loan_granted'])) ?>
                            </td>
                            
                            <td class="px-4 py-3 text-[14px] font-black text-slate-800 text-right border-r border-slate-100">
                            ...
                            <td class="px-4 py-3 text-[14px] font-black text-green-600 text-right border-r border-slate-100">
                                <div class="flex justify-between items-center">
                            ...
                            <td class="px-4 py-3 text-[14px] font-black text-red-800 text-right border-r border-slate-100">
                                <div class="flex justify-between items-center">
                                    <span class="text-[14px] text-slate-400 font-bold">₱</span>

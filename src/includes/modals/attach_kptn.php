<div id="attachKptnModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 overflow-hidden transform transition-all">
        ...
        <div class="bg-amber-50 border-b border-amber-100 px-8 py-5 flex justify-between items-center">
            <h2 class="text-amber-900 uppercase font-black text-sm tracking-widest">Verify & Activate Loan</h2>
            <button onclick="closeModal('attachKptnModal')" class="text-amber-400 hover:text-amber-800 transition-colors">
            ...
            <form id="attachKptnForm" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="ak_kptn_number" name="kptn_number" required placeholder="e.g. KPTN-2026-88192" 
                        class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-xl px-4 py-3 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all placeholder:text-slate-300">
                </div>
                ...
                <div>
                    ...
                    <input type="file" id="ak_kptn_receipt" name="kptn_receipt" required accept="image/png, image/jpeg, application/pdf"
                        class="w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-full file:border-0 file:text-[11px] file:font-bold file:uppercase file:tracking-wider file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 transition-all cursor-pointer border border-slate-200 rounded-xl p-1 bg-slate-50">
                ...
                <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                    ...
                    <button type="submit" id="btnSubmitKptn" class="px-6 py-3 bg-amber-500 hover:bg-amber-600 text-white text-[12px] font-bold uppercase tracking-widest rounded-full shadow-lg shadow-amber-500/30 transition-all active:scale-95">Verify & Activate</button>
                </div>
            </form>
        </div>
    </div>

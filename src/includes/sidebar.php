...
<aside id="sidebar" class="w-52 bg-[#e11d48] text-white flex flex-col transition-all duration-300 ease-in-out z-10 h-full sticky top-0 overflow-x-hidden">
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
        <ul class="space-y-1 py-4">
            ...
            <li class="<?= ($currentPage ?? '') === 'dashboard' ? 'bg-black/20 border-l-4 border-white' : '' ?>">
                <a href="<?= $baseUrl ?>/dashboard/" class="flex items-center gap-9 px-3 py-2 hover:bg-black/10 transition-all group">
            ...
            <li class="<?= ($currentPage ?? '') === 'upload' ? 'bg-black/20 border-l-4 border-white' : '' ?>">
                <a href="<?= $baseUrl ?>/upload/" class="flex items-center gap-9 px-3 py-2 hover:bg-black/10 transition-all group">
                    <svg class="w-6 h-6 opacity-80 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li class="<?= ($currentPage ?? '') === 'borrowers' ? 'bg-black/20 border-l-4 border-white' : '' ?>">
                <a href="<?= $baseUrl ?>/borrowers/" class="flex items-center gap-9 px-3 py-2 hover:bg-black/10 transition-all group">
                    <svg class="w-6 h-6 opacity-80 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li class="relative">
                <button type="button" onclick="handleReportsClick(event)" class="w-full flex items-center gap-4 px-3 py-2 hover:bg-black/10 transition-all focus:outline-none cursor-pointer group">
                ...
                <ul id="reports-menu" class="max-h-0 overflow-hidden bg-black/10 transition-all duration-300 ease-in-out">
                    <li><a href="<?= $baseUrl ?>/reports/deduction/" class="block pl-10 pr-6 py-3 text-[14px] font-bold tracking-widest hover:bg-white/10 border-b border-white/5 uppercase">Deduction</a></li>
                    <li><a href="<?= $baseUrl ?>/reports/ledger/" class="block pl-10 pr-6 py-3 text-[14px] font-bold tracking-widest hover:bg-white/10 border-b border-white/5 uppercase">Ledger</a></li>
                    <li><a href="<?= $baseUrl ?>/reports/running_receivables/" class="block pl-10 pr-6 py-3 text-[14px] font-bold tracking-widest hover:bg-white/10 uppercase">Running Receivables</a></li>
                </ul>
            </li>
            ...
            <li class="<?= ($currentPage ?? '') === 'borrower-mgt' ? 'bg-black/20 border-l-4 border-red-500' : '' ?>">
                <a href="<?= $baseUrl ?>/borrower-mgt/" class="flex items-center gap-9 px-3 py-2 hover:bg-black/10 transition-all group">
                    ...
                    <span class="sidebar-text text-[14px] font-bold tracking-widest text-red-400 uppercase whitespace-nowrap">Wipe Data</span>                
                </a>
            </li>

            <li class="<?= ($currentPage ?? '') === 'user-mgt' ? 'bg-black/20 border-l-4 border-white' : '' ?>">
                <a href="<?= $baseUrl ?>/user-mgt/" class="flex items-center gap-9 px-3 py-2 hover:bg-black/10 transition-all group">
                    <svg class="w-6 h-6 opacity-80 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                    </svg>    
            ...
            <li class="mt-4 border-t border-white/10">
                <a href="<?= $baseUrl ?>/actions/logout.php" class="flex items-center gap-9 px-3 py-2 hover:bg-black/10 transition-all group">
                    <svg class="w-6 h-6 opacity-80 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-6 bg-black/10 flex gap-24 items-center whitespace-nowrap overflow-hidden min-w-[256px] shrink-0 border-t border-white/10">
        <span class="sidebar-text text-[14px] font-bold tracking-widest uppercase truncate max-w-[150px]" title="<?= htmlspecialchars($userName) ?>">

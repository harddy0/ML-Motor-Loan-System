...
if (!isset($currentPage)) {
    ...
    if     (str_contains($uri, '/upload/payroll'))        $currentPage = 'upload-payroll';
    elseif (str_contains($uri, '/upload/ledger'))         $currentPage = 'upload-ledger';
    elseif (str_contains($uri, '/reports/deduction'))     $currentPage = 'reports-deduction';
    elseif (str_contains($uri, '/reports/ledger'))        $currentPage = 'reports-ledger';
        ...
        <ul class="space-y-1 py-1">
            ...
            <li class="relative <?= $isUploadActive ? 'border-l-4 border-white' : 'border-l-4 border-transparent' ?> transition-colors">
                <button type="button" onclick="handleUploadsClick(event)" class="w-full flex items-center justify-between px-5 py-1 hover:bg-black/10 transition-all focus:outline-none cursor-pointer">
                    ...
                    <svg id="uploads-arrow" class="w-4 h-4 sidebar-text opacity-70 <?= $isUploadActive ? 'rotate-180' : '' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="transition: transform 300ms cubic-bezier(0.4, 0, 0.2, 1);">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
                <ul id="uploads-menu" class="overflow-hidden bg-black/20 shadow-inner" style="max-height: <?= $isUploadActive ? '200px' : '0px' ?>; transition: max-height 300ms cubic-bezier(0.4, 0, 0.2, 1);">
                    <li>
                        <a href="<?= $baseUrl ?>/upload/payroll/" class="block pl-[3.25rem] pr-6 py-1 text-xs font-bold tracking-wider uppercase border-b border-white/5 transition-colors <?= $currentPage === 'upload-payroll' ? 'bg-black/20 text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' ?>">
                            Payroll Deduction
                        </a>
                    </li>
                    <li>
                        <a href="<?= $baseUrl ?>/upload/ledger/" class="block pl-[3.25rem] pr-6 py-1 text-xs font-bold tracking-wider uppercase transition-colors <?= $currentPage === 'upload-ledger' ? 'bg-black/20 text-white' : 'text-white/80 hover:bg-white/10 hover:text-white' ?>">
                            Ledger
                        </a>
                    </li>
                </ul>
            ...
            <li class="relative <?= $isReportsActive ? 'border-l-4 border-white' : 'border-l-4 border-transparent' ?> transition-colors">
                <button type="button" onclick="handleReportsClick(event)" class="w-full flex items-center justify-between px-5 py-1 hover:bg-black/10 transition-all focus:outline-none cursor-pointer">
                    ...
                    <svg id="reports-arrow" class="w-4 h-4 sidebar-text opacity-70 <?= $isReportsActive ? 'rotate-180' : '' ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="transition: transform 300ms cubic-bezier(0.4, 0, 0.2, 1);">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
...
if (sidebar) {
    sidebar.addEventListener('mouseenter', () => { isHoveringOver = true; });
    sidebar.addEventListener('mouseleave', () => { isHoveringOver = false; });
}

function expandSidebarOnHover() { expandSidebar(); }

function collapseSidebarOnLeave() {
    if (!isSidebarPinned) {
        setTimeout(() => { if (!isHoveringOver) collapseSidebar(); }, 100);
    }
}
